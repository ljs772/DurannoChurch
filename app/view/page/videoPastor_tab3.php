			...
			<div class="service-title col-md-12 text-center">
				<div>
					...
					if (count ( $pastorVideo ) > 0) {
						...
						foreach ( $pastorVideo as $pastorVideo_data ) {
							echo '<div>' . $pastorVideo_data ['title'] . " " . $pastorVideo_data ['path'].'</div>';
						}
				...
				if ($start_page >= $page_list_size) {
					...
					echo "<div id='page_list'><span id='page_prev' onclick = 'page_change(" . $prev_list . ");'>&&&&</span>";
				}
				
				for($i = $start_page; $i <= $end_page; $i ++) {
					$page = ($i - 1) * $page_size;
					if ($first_no != $page) {
						echo "<span id='page_num' onclick = 'page_change(" . $i . ");'>";
					}
					echo "<label>".$i."<label>";
					if ($first_no != $page) {
						echo "</span>";
				...
				if ($total_page > $end_page) {
					...
					echo "<span id='page_next' onclick = 'page_change(" . $next_list . ");'>****</span></div>";
				}
	        ...
	        $html_temp = '';
					...
					if (oData.results['pastorVideo'].length > 0) {
						...
						for ($i =0; oData.results['pastorVideo'].length > $i ;$i++) {
							$html_temp += '<div>' + oData.results['pastorVideo'][$i]['title'] + " " + oData.results['pastorVideo'][$i]['path']+"</div>";
						}
				...
				//	$prev_list = $start_page -  $page_list_size;
					...
					$html_temp += "<div id='page_list'><span  id='page_prev' onclick = 'page_change("+$prev_list+ ");'>&&&&</span>";
				}
				
				for($i = $start_page; $i <= $end_page; $i ++) {
					$page = ($i - 1) * $page_size;
					if ($first_no != $page) {
						$html_temp += "<span id='page_num' onclick = 'page_change(" + $i + ");'>";
					}
					$html_temp += $i;
					if ($first_no != $page) {
						$html_temp += "</span>";
				...
				if ($total_page > $end_page) {
					...
					$html_temp += "<span  id='page_next' onclick = 'page_change("+ $next_list+ ");'>****</span></div>";
				}				
			
	        
	        $('#pastorVideoList').show().html($html_temp).fadeIn();           
